...
class FuncionarioController extends Controller
{
    ...
    public function accessRules()
    {
        ...
        return array(
            array('allow', // allow authenticated user to perform 'create' and 'update' actions
                'actions' => array('cursosDisponibles', 'create',
                    'cursosDisponibles',
                    'exportarExcel',
                    'informacionCurso',
    ...
    public function actionInformacionCurso($id_curso_disponible)
    {


        $curso = CursoDisponible::model()->findByPk($id_curso_disponible);

        if ($curso != null) {

            $this->renderPartial('informacion_curso', array('curso' => $curso));
    ...
    public function actionRegistrarFuncionario($id_curso_disponible)
    {

        $cursoDisponible = CursoDisponibleServidorPublico::model()->findByAttributes(
            array('id_curso_disponible' => $id_curso_disponible,
                'id_servidor_publico' => Yii::app()->user->id));

        if ($cursoDisponible == null) {

            $curso = CursoDisponible::model()->findByPk($id_curso_disponible);

            $cursoDisponible = new CursoDisponibleServidorPublico();
            $cursoDisponible->id_servidor_publico = Yii::app()->user->id;
            $cursoDisponible->id_curso_disponible = $id_curso_disponible;
            $cursoDisponible->fecha_inicio = $curso->fecha_inicio;
            ...
            if ($cursoDisponible->save()) {
                ...
                echo CJSON::encode(array('respuesta' => 'El usuario Se registró con exíto'));

            }
            else{
                echo CJSON::encode(array('respuesta' => 'Ocurrió un error, por favor intenta más tarde'));
            }
        } else {
            echo CJSON::encode(array('respuesta' => 'El usuario ya está registrado'));
    ...
    public function actionCursosDisponibles()
    {
        ...
        $this->render('cursos_disponibles', array(
    ...
    public function actionIndex()
    {
        ...
        $servidorPublico = ServidorPublico::model()->findByPk(Yii::app()->user->id);
