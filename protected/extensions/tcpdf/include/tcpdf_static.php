...
class TCPDF_STATIC
{
    ...
    public static function getAnnotOptFromJSProp($prop, &$spot_colors, $rtl = false)
    {
        ...
        // buttonAlignX: Controls how space is distributed from the left of the button face with respect to the icons.
        if (isset($prop['buttonAlignX'])) {
            $opt['mk']['if']['a'][0] = $prop['buttonAlignX'];
        }
        // buttonAlignY: Controls how unused space is distributed from the bottom of the button face with respect to the icons.
        if (isset($prop['buttonAlignY'])) {
            $opt['mk']['if']['a'][1] = $prop['buttonAlignY'];
        }
        // buttonFitBounds: If true, the extent to which the icons may be scaled is set to the bounds of the button field.
        if (isset($prop['buttonFitBounds']) AND ($prop['buttonFitBounds'] == 'true')) {
            $opt['mk']['if']['fb'] = true;
        }
        // buttonScaleHow: Controls how the icons is scaled (if necessary) to fit inside the button face.
        ...
        // buttonScaleWhen: Controls when an icons is scaled to fit inside the button face.
        ...
        // buttonPosition: Controls how the text and the icons of the button are positioned with respect to each other within the button face.
