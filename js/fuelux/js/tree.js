...
!function ($) {
	...
	Tree.prototype = {
		...
		selectItem: function (el) {
			...
			if (this.options.multiSelect) {
				...
				$all.removeClass('tree-selected')
					.find('i').removeClass('icons-ok').addClass('tree-dot');
				data.push($el.data());
			}
			...
			if (this.options.selectable) {
				...
				if($el.hasClass('tree-selected')) {
					...
					$el.find('i').removeClass('icons-ok').addClass('tree-dot');
				} else {
					$el.addClass ('tree-selected');
					$el.find('i').removeClass('tree-dot').addClass('icons-ok');
					if (this.options.multiSelect) {
						data.push( $el.data() );
					}
				}
		...
		selectFolder: function (el) {
			...
			if ($el.find('.icons-folder-close').length) {
				eventType = 'opened';
				classToTarget = '.icons-folder-close';
				classToAdd = 'icons-folder-open';
				...
				eventType = 'closed';
				classToTarget = '.icons-folder-open';
				classToAdd = 'icons-folder-close';
			...
			$parent.find(classToTarget).eq(0)
				.removeClass('icons-folder-close icons-folder-open')
				.addClass(classToAdd);
		...
		collapse: function () {
			...
			// find open folders
			this.$element.find('.icons-folder-open').each(function () {
				// update icons class
				var $this = $(this)
